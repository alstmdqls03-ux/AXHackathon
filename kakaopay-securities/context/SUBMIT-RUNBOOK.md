...
2. 0단계대로 submission.zip **재생성** (이전 zip 재사용 금지 — 로그 최신화).
   **로그 최신성 검증(필수)**: zip에 들어갈 세션 로그가 최신 작업 이후 시점까지 담겼는지 확인 — 예: `unzip -p submission.zip 'logs/claude-code/*.jsonl' | grep -c 'flows'` 가 0이면 로그가 구버전 스냅샷이다(AI 심사 모의채점에서 실제 발견된 감점 요인: 문항 주장↔zip 로그 불일치). 마지막 주요 작업 키워드로 grep 히트를 확인한 뒤 업로드한다.
